...
### Infra — Supabase Gateway

```typescript
// features/booking/infra/supabase-appointment.gateway.ts
@Injectable()
export class SupabaseAppointmentGateway extends AppointmentGateway {
  ...
  getAll(): Observable<Appointment[]> {
    return from(
      this.supabase.client.from(TABLE_NAMES.APPOINTMENTS).select('*').limit(100)
    ).pipe(map(({ data, error }) => {
  ...
  getById(id: string): Observable<Appointment> {
    return from(
      this.supabase.client.from(TABLE_NAMES.APPOINTMENTS).select('*').eq('id', id).single()
    ).pipe(map(({ data, error }) => {
      if (error) throw error;
      return data as Appointment;
    }));
  ...
  create(data: Omit<Appointment, 'id'>): Observable<Appointment> {
    return from(
      this.supabase.client.from(TABLE_NAMES.APPOINTMENTS).insert({ ...data }).select().single()
...
export const appConfig: ApplicationConfig = {
  providers: [    provideRouter(routes, withComponentInputBinding()),    provideHttpClient(withFetch()),    { provide: AppointmentGateway, useClass: SupabaseAppointmentGateway },  ]};
```  
...
- Preconnect : `<link rel="preconnect" href="your-supabase-url" />`  
...
export class AppointmentStore {
  ...
  async findAll() {
    const { data, error } = await this.supabase.client.from(TABLE_NAMES.APPOINTMENTS).select('*').limit(100);
    if (error) throw error;
...
```typescript  
export const appConfig: ApplicationConfig = {
  providers: [    provideRouter(      routes,      withComponentInputBinding(),      withViewTransitions(),      withInMemoryScrolling({        anchorScrolling: 'enabled',        scrollPositionRestoration: 'enabled',      }),    ),    provideHttpClient(withFetch()),    { provide: AppointmentGateway, useClass: SupabaseAppointmentGateway },  ]};
```  
