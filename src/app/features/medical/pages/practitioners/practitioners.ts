...
@Component({
  ...
  template: `
    <header class="flex items-center justify-between">
      <div>
        <h2 class="text-2xl font-bold text-text-primary">
          {{ 'medical.practitioner.title' | transloco }}
        </h2>
        <p class="mt-1 text-sm text-text-muted">
          {{ 'medical.practitioner.subtitle' | transloco }}
        </p>
      </div>
      <button
        type="button"
    ...
    <section
      [attr.aria-label]="'medical.practitioner.listLabel' | transloco"
      class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"
    >
      @for (practitioner of practitioners(); track practitioner.id) {
        <article
          class="group relative overflow-hidden rounded-xl border border-border bg-surface transition hover:border-ib-blue/30 hover:shadow-lg hover:shadow-ib-blue/5"
        >
          <div class="p-5">
            <div class="flex items-center justify-between mb-3">
              ...
              <span
                class="rounded-full bg-ib-purple/15 px-2 py-0.5 text-xs font-medium text-ib-purple"
              >
            ...
            <dl class="ml-10 grid grid-cols-1 gap-1 text-sm">
              @if (practitioner.phone) {
                <div>
                  <dt class="text-text-muted text-xs">
                    {{ 'medical.practitioner.phone' | transloco }}
                  </dt>
              ...
              @if (practitioner.email) {
                <div>
                  <dt class="text-text-muted text-xs">
                    {{ 'medical.practitioner.email' | transloco }}
                  </dt>
              ...
              @if (practitioner.address) {
                <div class="mt-1">
                  <dt class="text-text-muted text-xs">
                    {{ 'medical.practitioner.address' | transloco }}
                  </dt>
              ...
              @if (practitioner.bookingUrl) {
                <div class="mt-1">
                  <dt class="text-text-muted text-xs">
                    {{ 'medical.practitioner.bookingLabel' | transloco }}
                  </dt>
                  <dd>
                    <a
                      ...
                      target="_blank"
                      rel="noopener"
                      class="text-xs text-ib-purple hover:underline"
                      >{{ 'medical.practitioner.bookAppointment' | transloco }}</a
                    >
                  </dd>
                </div>
              }
        ...
        >
          ...
          <p class="text-xs text-text-muted mt-1">
            {{ 'medical.practitioner.emptyHint' | transloco }}
          </p>
        </div>
      }
    </section>
    ...
    <app-modal-dialog
      #createModal
      [title]="'medical.practitioner.modalCreateTitle' | transloco"
      (closed)="onModalClosed()"
    >
      @if (createModal.isOpen()) {
        <app-practitioner-form
          (submitted)="createPractitioner($event)"
    ...
    <app-modal-dialog
      #editModal
      [title]="'medical.practitioner.modalEditTitle' | transloco"
      (closed)="onModalClosed()"
    >
      @if (editModal.isOpen()) {
        <app-practitioner-form
          [initial]="selectedPractitioner()"
          (submitted)="updatePractitioner($event)"
...
export class Practitioners {
  ...
  protected async deletePractitioner(id: string) {
    if (!(await this.confirm.delete(this._i18n.translate('medical.practitioner.deleteEntityName'))))
      return;
    try {
