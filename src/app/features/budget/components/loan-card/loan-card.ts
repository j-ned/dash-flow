    ...
    >
      <div class="flex items-start justify-between mb-3">
        <div class="flex items-center gap-3">
          <div class="flex h-10 w-10 items-center justify-center rounded-xl text-xs font-bold shrink-0" [class]="badgeClass()">
            {{ vm().pct | number: '1.0-0' }}%
          </div>
          <div>
        ...
        <div class="text-right">
          <span class="block text-lg font-mono font-bold" [class]="remainingClass()">{{ vm().loan.remaining | number: '1.2-2' }}<span class="text-sm ml-0.5">&euro;</span></span>
      ...
      <div class="grid grid-cols-3 gap-2 text-xs mb-3">
        ...
        <div class="rounded-lg bg-canvas p-2 border border-border/30">
          ...
          <p class="font-mono font-medium" [class]="remainingClass()">{{ vm().loan.remaining | number: '1.2-2' }}&euro;</p>
        </div>
      </div>
      ...
      <div class="h-2 rounded-full bg-hover overflow-hidden">
        <div class="h-full rounded-full transition duration-500 ease-out" [class]="barClass()" [style.width.%]="clampedPct()"></div>
      </div>
      ...
      <div class="mt-3 pt-3 border-t border-border/30">
        <div class="mb-2 flex items-center justify-between">
          ...
          @if (vm().entries.length > 0) {
            <button type="button" class="text-[10px] font-medium text-ib-cyan hover:underline" (click)="history.emit(vm().loan)">
...
export class LoanCard {
  ...
  protected readonly hoverClass = computed(() => (this.lent() ? 'hover:bg-ib-blue/3' : 'hover:bg-ib-orange/3'));
  protected readonly badgeClass = computed(() => (this.lent() ? 'bg-ib-blue/10 text-ib-blue' : 'bg-ib-orange/10 text-ib-orange'));
  protected readonly remainingClass = computed(() => (this.lent() ? 'text-ib-blue' : 'text-ib-red'));
  protected readonly barClass = computed(() => (this.lent() ? 'bg-ib-blue' : 'bg-ib-orange'));
  protected readonly repayClass = computed(() =>
    this.lent() ? 'bg-ib-blue/10 text-ib-blue hover:bg-ib-blue/20' : 'bg-ib-orange/10 text-ib-orange hover:bg-ib-orange/20',
  );
  protected readonly dateKey = computed(() => (this.lent() ? 'budget.loan.loanDate' : 'budget.loan.borrowDate'));
  protected readonly editAriaKey = computed(() => (this.lent() ? 'budget.loan.actions.editLentAria' : 'budget.loan.actions.editBorrowedAria'));
  protected readonly deleteAriaKey = computed(() => (this.lent() ? 'budget.loan.actions.deleteLentAria' : 'budget.loan.actions.deleteBorrowedAria'));
}
