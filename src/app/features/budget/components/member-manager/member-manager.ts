    ...
    <app-modal-dialog #modal [title]="'budget.members.title' | transloco" (closed)="resetForm()">
          ...
          @for (m of members(); track m.id; let i = $index) {
            <li class="flex items-center justify-between gap-3 px-3 py-2">
              <span class="flex min-w-0 items-center gap-2">
                <span class="inline-block h-3 w-3 shrink-0 rounded-full" [style.background-color]="color(i)"></span>
                <span class="truncate text-sm text-text-primary">{{ m.firstName }} {{ m.lastName }}</span>
              </span>
              <span class="flex shrink-0 items-center gap-1">
                <button type="button" class="rounded-md p-1.5 text-text-muted hover:bg-hover hover:text-text-primary transition-colors"
                        [attr.aria-label]="'budget.members.editAria' | transloco: { name: m.firstName }" (click)="startEdit(m)">
                  <app-icon name="pencil" size="14" />
                </button>
                <button type="button" class="rounded-md p-1.5 text-text-muted hover:bg-hover hover:text-ib-red transition-colors"
                        [attr.aria-label]="'budget.members.deleteAria' | transloco: { name: m.firstName }" (click)="remove(m)">
      ...
      <form (ngSubmit)="save()" class="space-y-3 border-t border-border pt-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="member-first" class="block text-sm font-medium text-text-muted mb-1">{{ 'budget.members.firstName' | transloco }} <span aria-hidden="true">*</span></label>
            <input id="member-first" type="text" [ngModel]="firstName()" (ngModelChange)="firstName.set($event)" name="firstName" maxlength="255"
                   class="w-full rounded-lg border border-border bg-raised px-3 py-2 text-sm text-text-primary" />
          </div>
          <div>
            <label for="member-last" class="block text-sm font-medium text-text-muted mb-1">{{ 'budget.members.lastName' | transloco }}</label>
            <input id="member-last" type="text" [ngModel]="lastName()" (ngModelChange)="lastName.set($event)" name="lastName" maxlength="255"
                   class="w-full rounded-lg border border-border bg-raised px-3 py-2 text-sm text-text-primary" />
          </div>
        </div>
        <div class="flex justify-end gap-2">
          @if (editingId()) {
            <button type="button" class="rounded-lg border border-border px-4 py-2 text-sm text-text-muted hover:bg-hover transition-colors" (click)="resetForm()">
...
export class MemberManager {
  ...
  protected async remove(member: Member): Promise<void> {
    if (!await this.confirm.confirm({
      ...
      message: this._i18n.translate('budget.members.deleteConfirmMessage', { name: `${member.firstName} ${member.lastName}` }),
