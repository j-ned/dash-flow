...
@Component({
  ...
  template: `
    ...
    >
      <span class="block h-3.5 w-5 overflow-hidden rounded-[2px] border border-border/60 leading-none" aria-hidden="true">
        @if (locale.isFrench()) {
          ...
          <svg viewBox="0 0 60 30" class="block h-full w-full">
            <clipPath id="locale-toggle-uk-mask"><path d="M30 15h30v15zv15H0zH0V0zV0h30z" /></clipPath>
            <path d="M0 0v30h60V0z" fill="#012169" />
            <path d="M0 0l60 30m0-30L0 30" stroke="#ffffff" stroke-width="6" />
            <path d="M0 0l60 30m0-30L0 30" clip-path="url(#locale-toggle-uk-mask)" stroke="#C8102E" stroke-width="4" />
            <path d="M30 0v30M0 15h60" stroke="#ffffff" stroke-width="10" />
            <path d="M30 0v30M0 15h60" stroke="#C8102E" stroke-width="6" />
          </svg>
        }
      </span>
      <span class="font-mono text-xs font-medium tracking-tight">{{ locale.isFrench() ? 'FR' : 'EN' }}</span>
    </button>
    <button
      type="button"
      (click)="theme.toggle()"
